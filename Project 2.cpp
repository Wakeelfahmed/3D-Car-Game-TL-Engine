...
// Define game states
enum currentState { Demo, Count_Down, Stage_0_Complete, Stage_1_Complete, Stage_2, Race_Complete };

// Define car states
enum carState { Hovering, WaitingForGo, Racing };

// Initialize game state to Demo
...
// Game models/assets
IModel* myCar, * Track_isles[isle_count], * Check_Points[CheckPoint_count];
...
// Function to print the game completed stage
...
// Function to print the backdrop, car speed, and game state
...
// Function to detect collision between car and objects
...
// Function to remove car rotation
...
// Function to update the car's position and rotation
void update_car()
{
	float bank, angle;
	if (!collision)
...
void main()
{
	...
	Check_Points[0] = checkPointMesh->CreateModel(0, 0, 0);
	Check_Points[1] = checkPointMesh->CreateModel(0, 0, 150);

	// Initialize checkpoint collision bounds
	...
	// Create and attach the camera to the car model
	ICamera* myCamera = myEngine->CreateCamera(kManual, 0.0f, 20.0f, -60.0f);

	// Load isle models
	IMesh* isleMesh = myEngine->LoadMesh("islestraight.x");
	Track_isles[0] = isleMesh->CreateModel(-10, 0, 40);
	Track_isles[1] = isleMesh->CreateModel(10, 0, 40);
	Track_isles[2] = isleMesh->CreateModel(10, 0, 53);
	Track_isles[3] = isleMesh->CreateModel(-10, 0, 53);

	// Initialize isle collision bounds
	...
	// Load hover car model and set its skin
	...
	while (myEngine->IsRunning())
	{
		...
		if (!gamePaused)
		{
			...
			for (int isle = 0; isle < isle_count; isle++)
